...
class Vaunix_attenuator(Instrument):
    ...
    def do_set_attenuation(self, value):
        logging.info( '{} : Setting the attenuation at {}dB'.format(self._serial_number, value))
        attenuation = int(((value-11)*120) / 6)
        self._vnx.fnLDA_SetAttenuationHR(self._Devices[self._device_number], attenuation)
    ...
    def do_get_attenuation(self):
        attenuation = self._vnx.fnLDA_GetAttenuationHR(self._Devices[self._device_number])
        return (attenuation*6) / 120.0 + 11
